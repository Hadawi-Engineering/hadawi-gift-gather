// Hadawi Gift Collection - Firebase Backend Version
// This file handles occasion data fetching from Firebase and ClickPay payment integration via Firebase Functions
...
// Handle payment form submission with Firebase Cloud Function
async function handlePayment(event) {
    ...
    try {
        ...
        // Call Firebase Cloud Function to initiate ClickPay payment
        const initiatePayment = httpsCallable(functions, 'initiateClickPayPayment');
        ...
        if (result.data.success && result.data.paymentUrl) {
            ...
            if (result.data.transactionRef) {
                sessionStorage.setItem('transactionRef', result.data.transactionRef);
            }
            
            // Redirect to ClickPay hosted payment page
            window.location.href = result.data.paymentUrl;
        } else {
            throw new Error(result.data.message || 'No payment URL received from ClickPay');
        }
